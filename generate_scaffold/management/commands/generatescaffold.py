...
from generate_scaffold.utils.cacheclear import clean_pyc_in_dir, reload_django_appcache
...
class Command(VerboseCommandMixin, BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option(
            ...
            help=_(
                "The name of the field used as a timestamp for date-based "
    ...
    def generate_scaffold(self, *args, **options):
        ...
        if self.timestamp_fieldname and not self.existing_model:
            raise CommandError(
                smart_str(_("The --timestamp-field option can only be used if --model " "is specified."))
            )
            ...
            raise CommandError(
                smart_str(
                    _(
                        ...
                        "in order for {1} to generate templates.".format(app_name, self.command_name)
                    )
                )
            )
        ...
        try:
            ...
            raise CommandError(smart_str(_("Could not import app with name: {0}".format(app_name))))
        ...
        if not self.existing_model:
            try:
                ...
                raise CommandError(smart_str(_("You must provide a model_name.")))
        else:
        ...
        model_views_filepath = os.path.join(app_views_dirpath, model_views_filename)

        # TODO - Append to views file if already exists
        if os.path.isfile(model_views_filepath):
            raise CommandError(smart_str(_("{0} already exists.".format(model_views_filepath))))
        ...
        if not model_field_args and not self.existing_model:
            ...
            raise CommandError(smart_str(_("Cannot generate model with no fields.")))

        for arg in [a for a in model_field_args if len(a) < 2]:
            raise CommandError(smart_str(_("No field type specified for model field: {0}".format(arg))))

        if not self.existing_model:
            models_generator = ModelsGenerator(app_name)
            try:
                rendered_model, rendered_model_name = models_generator.render_model(
                    model_name, model_field_args, self.is_timestamped
                )
            except GeneratorError as err:
                raise CommandError(smart_str(_("Could not generate model.\n{0}".format(err))))
        ...
        with FilesystemTransaction(self.dry_run, self) as transaction:
            ...
            if not self.existing_model and self.dry_run:
                ...
                with open(app_models_filepath, "r") as f:
                    code = compile(f.read() + rendered_model, "<string>", "exec")
                ...
                code_str = "generated_model = {0}().__class__".format(rendered_model_name)
            ...
            app_views_init_filepath = os.path.join(app_views_dirpath, "__init__.py")
            ...
            rendered_views = views_generator.render_views(generated_model, self.timestamp_fieldname)
            ...
            rendered_urls = urls_generator.render_urls(generated_model, self.timestamp_fieldname)
            ...
            ### Generate templates ###
            app_templates_root_dirpath = os.path.join(app_dirpath, "templates")
            transaction.mkdir(app_templates_root_dirpath)

            app_templates_app_dirpath = os.path.join(app_templates_root_dirpath, app_name)
            transaction.mkdir(app_templates_app_dirpath)

            model_templates_dirpath = os.path.join(app_templates_app_dirpath, slugify(model_name))
            ...
            rendered_templates = templates_generator.render_templates(
                generated_model, model_templates_dirpath, self.timestamp_fieldname
            )
