...
FIELD_ALIASES = {
    ...
    "positivesmallintegerfield": ["positivesmallint", "positivesmallinteger", "positivesmallintegerfield"],
    ...
    def get_timestamp_field(self, model, timestamp_fieldname=None):
        if timestamp_fieldname:
            try:
                ...
                raise GeneratorError('{0} does not have a field named "{1}"'.format(model, timestamp_fieldname))
            if type(timestamp_field) not in [DateField, DateTimeField]:
                raise GeneratorError(
                    "{0} is not a DateField or a DateTimeField, it cannot "
                    "be used as a timestamp field.".format(timestamp_field)
                )
